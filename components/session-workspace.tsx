...
export function SessionWorkspace() {
  ...
  const loadSession = useCallback(async (path: string) => {
    ...
    try {
      setSession(await sessionRequest<SessionDetail>(`/api/sessions/document?path=${encodeURIComponent(path)}`));
    } catch (error) {
            ...
            <div className="border-b px-4 py-3">
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                <div className="flex rounded-lg border bg-muted/50 p-0.5">
                  <button onClick={() => selectViewer("transcript")} className={cn("flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[11px] font-medium", viewer === "transcript" && "bg-white shadow-sm")}><MessageSquareText className="size-3" />Transcript</button>
                  <button onClick={() => selectViewer("raw")} className={cn("flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[11px] font-medium", viewer === "raw" && "bg-white shadow-sm")}><Braces className="size-3" />Raw JSONL</button>
                </div>
